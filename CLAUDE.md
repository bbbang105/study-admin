...
| `packages/web/middleware.ts` | 라우트 보호 (인증 체크만, 관리자 권한은 admin layout에서 처리) |
...
- **미들웨어**: `@supabase/ssr`의 `updateSession()`으로 세션 자동 갱신 + 인증 체크만 (관리자 권한은 레이아웃/API에서 처리)
- **관리자**: `(admin)/layout.tsx`에서 `/api/admin/check` 호출 → 접근 거부 다이얼로그, API는 `withAdminAuth` 래퍼로 서버사이드 권한 체크
